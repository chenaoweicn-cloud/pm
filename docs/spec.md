...
**操作**：
- 创建 / 编辑 / 软删除
- **归档**：`status='archived'`，记录 `archived_at`，项目从侧边栏消失
- **重启**：将归档项目恢复为 `active`
...
**全局布局**：
- **侧边栏（常驻）**：
  - 顶部导航：今日待办、历史回顾、搜索、回收站、设置
  - 中部：**活跃项目列表**（直接缓解"不知道手上有什么项目"的痛点）
  - 底部：归档项目的折叠入口
- **主区**：根据当前选中项切换视图
...
| 跨项目 看板 | 跨项目 | 按 3 态分三列，卡片上标注所属项目名（用项目类型做视觉辅助区分） |
...
### 8.1 新建任务
1. 任一视图点击"+ 新建任务"按钮（或 Cmd+N）
2. 弹出模态框
3. 必填：项目（若在项目上下文则自动预选）、任务名
4. 选填字段折叠默认收起，点击展开
5. 回车/确定提交 → 新任务立即出现在当前视图

### 8.2 写周报流程
1. 周五下午点击侧边栏"历史回顾"
2. 默认展示"本周"区间
3. 查看按项目分组的"本周完成任务 + 进行中任务"
4. 在外部工具（飞书/企业邮箱）手写周报，参考此页内容
5. 可切换到"本月"做月报，或用自定义区间回顾任意时段

### 8.3 归档与重启
1. 项目详情页 → 顶部操作菜单 → 归档
2. 确认弹窗 → 归档
3. 项目从侧边栏消失，进入"归档项目"折叠组
4. 需要重启：侧边栏归档组 → 选项目 → "重启"按钮

### 8.4 项目关联（二期从一期衍生）
1. 新建项目对话框 → 勾选"关联到已有项目"
2. 选择源项目 + 关系类型（successor / related）
3. 两个项目的详情页都会显示该关联

### 8.5 全局搜索
1. 任意界面按 Cmd+K
2. 输入关键字，实时展示匹配的项目和任务
3. 键盘上下选择，回车跳转到对应项目/任务详情
...
- 完整快捷键方案（V1 只做 Cmd+K 搜索、Cmd+N 新建任务）
- UI 主题（V1 跟随系统，shadcn/ui 默认配色；后续可做主题切换）
- 多语言（V1 只做中文）
